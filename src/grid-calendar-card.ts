...
export const CARD_TAG = "grid-calendar-card";
...
export class GridCalendarCard extends LitElement {
  /** HA wiring */
  hass: any;
  ...
  static async getStubConfig(hass: any): Promise<GridCalendarCardConfig> {
  ...
  render() {
    ...
    return html`
      <ha-card>
        ...
        <div class="footer">Grid Calendar Card v${VERSION}</div>
        ${this._renderDialog()}
      </ha-card>
    `;
  }
    ...
    for (let d = 0; d < 7; d++) {
      ...
      out.push(html`
        <div class=${classes.join(" ")} style=${`grid-column:${2 + d}/${3 + d}`}>
          ...
          ${allDay}
          <div class="body" style=${`height:${columnHeight}px; position:relative; ${bg}`}>${timed}${nowLine}</div>
        </div>
      `);
    }
...
if (!customElements.get(CARD_TAG)) customElements.define(CARD_TAG, GridCalendarCard as any);
